...
export default function BasketItem({ book, deleteBook, type, finishShopping }) {
      ...
      <div className="basketItem">
        ...
        <div className="basketItem-item">
          <div>
            {book.volumeInfo.hasOwnProperty("imageLinks") ? (
              <img src={imageLinks.thumbnail} alt="bookCover" />
            ) : (
        ...
        <div className="basketItem-price">
          {type === "buy" ? (
            ...
            <p className="brownPrice">
              <MdAccessTime /> 1 month
            </p>
          )}
          <button
            className="btnBasket"
            onClick={() => finishShopping(book, type)}
          >
            {type === "buy" ? "Buy" : "Rent"}
          </button>
          <button className="btnBasket" onClick={() => deleteBook(book.etag)}>
            Delete
          </button>
        </div>
      </div>
